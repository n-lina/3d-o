...
const Appendages = (props) => {
    ...
    if (modelStore.flat_bottom){
      ...
      bottoms.push(<Bottom key={key} caption={"bottom"} flat={true} circ={circ} lid={false} formObject={formObject}/>) // circ = circumfrence
      key += 1
    }
    else if (formObject.model === "basket"){
      const circ = modelStore.getDimensions()[modelStore.getDimensions().length-1][0]
      bottoms.push(<Bottom key={key} caption={"bottom"} flat={false} circ={circ} lid={false} formObject={formObject}/>)
      key += 1
    }
    if (modelStore.lid){
    ...
    if (modelStore.arms){
      const circ = modelStore.getDimensions()[modelStore.getDimensions().length-1][0]
      sections.push(<Arm key={key} size={circ} formObject={formObject}/>)
      key += 1
      sections.push(<Arm key={key} size={circ} formObject={formObject}/>)
      key += 1
    }
    if (modelStore.ears !== ""){
      ...
      if (modelStore.ears === "bear"){
        sections.push(<Rim key={key} half={true} caption={"bear ears x 2"} circ={circ}/>) // static picture
      }
      ...
      //   bottoms.push(<Sphere key={key} caption={"round ears x 2"} size={circ} formObject={formObject}/>)
      //   key += 1
      //   bottoms.push(<Sphere key={key} caption={"round ears x 2"} size={circ} formObject={formObject}/>)
      // }
    ...
    if (modelStore.side_handles){
      const circ = modelStore.getDimensions()[0][0]
      sections.push(<Rim key={key} half={true} caption={"side handles"} circ={circ}/>) // static picture
      key += 1
    }
    if (modelStore.top_rim){
      sections.push(<Rim key={key} half={false} caption={"top rim"} circ={modelStore.getDimensions()[0][0]}/>) // static picture
      key += 1
    }
    if (modelStore.bottom_rim){
      const circ = modelStore.getDimensions()[modelStore.getDimensions().length-1][0]
      sections.push(<Rim key={key} half={false} caption={"bottom rim"} circ={circ}/>) // static picture
      key += 1
    }
