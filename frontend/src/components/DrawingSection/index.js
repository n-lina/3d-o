...
const DrawingSection = (props) => {
  const { rows_till_this_section, caption, specialTop, specialBottom, formObject, DrawingSectionModel } = props;
  ...
  for (let i = 0; i < DrawingSectionModel.height; i++) {
    ...
    if (i % 2 === 1) {
      ...
      rows.push(<Row key={i} formObject={formObject} RowModel={DrawingSectionModel.drawingSectionData[i]} rows_till_this_section={rows_till_this_section} />);
    }
    else {
      rows.push(<Row key={i} formObject={formObject} RowModel={DrawingSectionModel.drawingSectionData[i]} rows_till_this_section={rows_till_this_section} />);
    }
